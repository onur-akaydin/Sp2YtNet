...
namespace Sp2YtNet
{
    public static class SpotifyOperations
    {
        ...
        public static async Task<SpotifyClient> LoginAndGetSpotifyClientAsync()
        {
            ...
            CheckIfSpotifyCredentialsValid();
        ...
        private static void CheckIfSpotifyCredentialsValid()
        {
            if (Sp2YtNetAuthSecrets.Instance.SpotifyClientId.StartsWith("PUT")
                            ||
                            Sp2YtNetAuthSecrets.Instance.SpotifyClientSecret.StartsWith("PUT"))
            {
                Console.WriteLine($"Make the configuration of {Sp2YtNetAuthSecrets.SECRETS_FILE_PATH} first, then try again.");
                Environment.Exit(1);
            }
        }        
